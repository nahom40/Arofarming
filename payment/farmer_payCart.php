...
$sql1 = "SELECT `order_id`, `payer_C`, `payer_F`, `Amount`, `product_id`, `product_name`, `quaintly`, `Pay_to`, `status`, `merch_id` FROM `orders` WHERE `payer_F`='$F_id' and `status`='not_payed'";
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    if (isset($userPayments[$payerF])) {
        // Add the item to the existing payment details
        $userPayments[$payerF]['items'][] = $newItem;
    } else {
...
<body>
    ...
    <?php foreach ($userPayments as $userPayment) { ?>
        <form method="post" action="https://test.yenepay.com/">
            ...
            <input type="hidden" name="successUrl" value="http://localhost/n7/Afro_farming/payment/Fcart_check.php">
            ...
            <button type="submit">Pay with cart for User <?php echo $userPayment['payerF']; ?></button>
        </form>
        <br>
    <?php } ?>
</body>
