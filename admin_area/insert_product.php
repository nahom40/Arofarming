...
if (isset($_POST['insert_product'])) {
    ...
    if (
        ...
        || $product_catagory == '' || $product_brand == '' || $product_price == '' ||
        $merch_id == ''
    ) {
        ...
        // Check if the image files were successfully uploaded.
        if (
            !empty($product_image1) && !empty($product_image2) && !empty($product_image3) &&
            file_exists("./product_images/$product_image1") &&
            file_exists("./product_images/$product_image2") &&
            file_exists("./product_images/$product_image3")
        ) {
            $insert_product = "INSERT INTO `farmer_products` (
                `A_id`, `P_name`, `P_desc`, `PC_id`, `P_image1`, `P_image2`, `P_image3`,
                `P_key`, `P_date`, `P_price`, `PL_id`, `merch_id`
            )
            VALUES (
                '$thedata', '$product_title', '$product_description', '$product_catagory', '$product_image1',
                '$product_image2', '$product_image3', '$product_keyword', NOW(), '$product_price',
                '$product_brand', '$merch_id'
            )";
        ...
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <div class="form-outline mb-4 w-50 m-auto">
                ...
                <select name="merchant_id" id="product_brand" class="form-select">
                    ...
                    <?php
                   $select_query = "SELECT * FROM `merchant_ids` WHERE  `A_id`='$thedata'";
                   $result_query = mysqli_query($conn, $select_query);
                   while ($row = mysqli_fetch_assoc($result_query)) {
